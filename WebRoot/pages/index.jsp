...
<body>
		...
		<div class="hm-inner clearfix">
			...
			<div class="hm-inner-r r">
							<div class="box">
								...
								<div class="picright">
									<a href="/bbs/to/userInfo.do">${sessionScope.userName}</a><a href="/bbs/to/sessionInvalidate.do">【注销】</a><a href="/bbs/to/toRegister.do">【新用户注册】</a>
								</div>
								</c:if>
							</div>
		...
		<div class="hm-inner">
			...
			<ul class="hm-bbs-nav border-lrb clearfix">
            	...
            	<c:forEach items="${requestScope.zones}" var="zone">
                	<li
                		<c:if test="${zoneId == null}">
                			<c:if test="${zone.isDef == 1}">
                				class="current"
                			</c:if>
                		</c:if>
                		<c:if test="${zoneId != null && zoneId == zone.zoneId}">
                			class="current"	
                		</c:if>>
	                	<a href="/bbs/to/index.do?zoneId=${zone.zoneId}"><em></em>${zone.zoneName}</a>
                    </li>
                </c:forEach>
			</ul>
			<!-- 主体部分 -->
						...
						<c:forEach items="${articles}" var="article">
							<li class="clearfix <c:if test='${article.isTop == 1}'>ding</c:if>">
								<div class="hm-index-title">
									<c:if test="${article.isTop == 1}"><i class="set-to-top">顶</i></c:if><a href="/bbs/article/articltDetail.do?articleId=${article.articleId}">${article.title}</a>
								</div>
								...
								<div class="hm-index-info l">
									<span class="article-username">${article.senderName}</span><span
										class="post-time"><fmt:formatDate value="${article.sendTime}" pattern="yyyy年MM月dd日 HH:mm:ss"/></span>
								</div>
	...
	<!-- 发帖弹出框 -->
	<form action="/bbs/article/addArticle.do" method="post" onSubmit="return check()">
